					...
					function l(e) {
						...
						for (let n = 0; n < l.length; n++)
							(t[l[n].dataset.lang] = l[n].value),
								(l[n].parentNode.querySelector('code').innerHTML = hljsWeb
									.highlightAuto(l[n].value)
									.value.replace(/\n/g, '<br/>')),
								l[n].addEventListener('scroll', (e) => {
									e.target.parentNode.querySelector('pre').scrollTo(0, l[n].scrollTop);
								}),
								l[n].addEventListener('keyup', (e) => {
									(t[e.target.dataset.lang] = e.target.value),
										(e.target.parentNode.querySelector('code').innerHTML = hljsWeb
...
function runCode(code) {
	...
	try {
		...
		let op =
			'<span>> ' +
			context.store.map((e) => e.map((a) => JSON.stringify(a)).join(' ')).join('\n</span><span>> ');
		return op;
	} catch (e) {
		let op =
			'<span>> ' +
	...
	if (lang[0] == '>' && ['javascript', 'js'].indexOf(lang.slice(1)) != -1) {
        ...
        <div>
          <label for="html">
            HTML
            <div code>
              <textarea name="html" data-lang="html">${doc.html || ''}</textarea>
              <pre>
                <code id="html"></code>
              </pre>
            </div>
